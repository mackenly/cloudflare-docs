...
<PackageManagers
	type="create"
	pkg="cloudflare@latest auth-js-d1-example"
...
With a domain [created and verified](https://resend.com/docs/dashboard/domains/introduction) within Resend, add a new Secret to your Worker containing an email from a verified domain such as `noreply@auth-js-d1-example.example.com`.
...
The D1 adapter requires that tables be created within your database. It recommends using using the exported up() method to complete this. Within `src/app/api/` create a directory called `setup` containing a file called `route.ts`. Within this route handler, add the following code:
...
:::note[Clean up]
Running this multiple times won't hurt anything, but it's a good idea to remove this route from your production code once you've run it.
:::
...
<PackageManagers pkg="shadcn@latest init -d" />
...
```sh title="Preview your application"
npm run preview
```
